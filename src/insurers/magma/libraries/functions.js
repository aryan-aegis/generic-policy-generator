...
import { mobile } from '../../../shared/constants.js';
...
const getCookies = async () => {
  let cookiesJSON;
  try {
    cookiesJSON = await fs.readFile('./cookies.json', { encoding: 'utf8' });
  } catch (err) {
    console.error('Error getting cookies');
  }

  if (cookiesJSON) return await JSON.parse(cookiesJSON);
};
...
const loginMagma = async (page, branch, pendingXHR) => {
  ...
  if (authCookies)
    ({ expires } = authCookies.find((cookie) => cookie.name === 'FedAuth'));

  const currentTimestamp = Math.floor(dayjs().unix());

  console.log({ authCookies, a: currentTimestamp > expires });

  if (!authCookies || currentTimestamp > expires) {
...
const setCustomerDetails = async (
  ...
  if (customerDetails.customerType === 'Individual') {
    const [salutation] = customerDetails.salutation.split('.');
    const dob = dayjs(customerDetails.dob).format('DD/MM/YYYY');

    // KYC process
    const customerFullName = `${customerDetails.firstName} ${
      customerDetails.lastName.includes('.') ? '' : customerDetails.lastName
    }`;
    ...
    await selectDropdownOptionByLabel(
      page,
      selectors.kycDocumentTypeDropdown,
      'AADHAAR'
    );
    await setElementValue(
      page,
      selectors.kycDocumentIdInput,
      customerDetails.aadhaar
    );
    await setElementValue(
      page,
      selectors.kycIdNameInput,
      customerFullName.trim()
    );
    await setElementValue(
      page,
      selectors.kycIdDobInput,
      dayjs(customerDetails.dob).format('DD-MM-YYYY')
    );
    await pendingXHR.waitForAllXhrFinished();
    ...
    const ckycMesssage = await page.evaluate(
    ...
    if (!ckycVerified) {
      ...
      await selectDropdownOptionByLabel(
        page,
        selectors.documentTypeDropdown,
        'PAN'
      );
      await selectDropdownOptionByLabel(
        page,
        selectors.supportingDocumentType,
        'AADHAAR(SUPPORTING)'
      );

      // Upload main file
      const uploadDocumentButton = await page.$(
        selectors.documentMainChooseFileButton
      );
      uploadDocumentButton.uploadFile(
        `${kycDocumentFolderPath}/${kycDocumentsFileNames.panFileName}`
      );
      ...
      const uploadSupportingDocumentButton = await page.$(
        selectors.supportingDocumentChooseFileButton
      );
      uploadSupportingDocumentButton.uploadFile(
        `${kycDocumentFolderPath}/${kycDocumentsFileNames.aadhaarFileName}`
      );
    ...
    await selectDropdownOptionByLabel(
      page,
      selectors.martialStatusDropdown,
      'Single'
    );
    await pendingXHR.waitForAllXhrFinished();
    // Set customer DOB
    ...
    if (!ckycVerified) {
      ...
      await page.waitForSelector(selectors.addressLine1Input, {
        visible: true
      });
      await setElementValue(
        page,
      ...
      await setElementValue(
        page,
        selectors.addressLine2Input,
        customerDetails.addressLine2
      );
      customerDetails.addressLine3
        ? await setElementValue(page, selectors.addressLine3Input)
        : null;
      // Enter pincode
      await setElementValue(
        page,
        selectors.pincodeInput,
      ...
      try {
        await selectDropdownOptionByLabel(
          page,
          selectors.pincodeAreasDropdown,
          insuranceDetails.insurer.data.area.toUpperCase()
        );
      } catch (error) {
        // error
      ...
      await setElementValue(
        page,
        selectors.mailingAddresslineInput1,
        customerDetails.addressLine1
      );
      await setElementValue(
        page,
        selectors.mailingAddresslineInput2,
        customerDetails.addressLine2
      );
      ...
      customerDetails.addressLine3
        ? await setElementValue(
            page,
            selectors.mailingAddresslineInput3,
            customerDetails.addressLine3
          )
        : await setElementValue(page, selectors.mailingAddresslineInput3, '');
      // Enter pincode
      await setElementValue(
        page,
        selectors.mailingPincodeInput,
        customerDetails.pincode
      );
      await pendingXHR.waitForAllXhrFinished();
      // Click search pincode details button
      ...
      try {
        await selectDropdownOptionByLabel(
          page,
          selectors.mailingPincodeAreasDropdown,
          insuranceDetails.insurer.data.area.toUpperCase()
        );
      } catch (err) {
        // Nothing
    ...
    await selectDropdownOptionByLabel(
      page,
      selectors.corporateCkycDocumentIdType,
      'CORPORATE PAN'
    );
    await setElementValue(
      page,
    ...
    await setElementValue(
      page,
      selectors.corporateDateOfIncorporationInput,
      dayjs(customerDetails.doi).format('DD-MM-YYYY')
    );
    await domClickElement(page, selectors.kycVerifyButton);
  ...
  if (!ckycVerified) {
    ...
    await setElementValue(
      page,
      selectors.supportingDocumentId,
      customerDetails.aadhaar
    );
  }
...
const setPolicyDetails = async (
  ...
  insuranceDetails.policyType !== 'od1tp5'
    ? await selectDropdownOptionByLabel(
        page,
        selectors.businessTypeDropdown,
        'Roll Over'
      )
    : null;
  ...
  await selectDropdownOptionByLabel(
    page,
    selectors.relationshipCodeDropdown,
    relationshipCode
  );
  await pendingXHR.waitForAllXhrFinished();
  ...
  if (insuranceDetails.policyType !== 'od1tp5') {
    ...
    await page.waitForSelector(selectors.previousInsurerDropdown, {
      visible: true
    });
    ...
    insuranceDetails.policyType === 'od1'
      ? await selectDropdownOptionByLabel(
          page,
          selectors.previousPolicyTypeDropdown,
          'Bundled Policy'
        )
      : await selectDropdownOptionByLabel(
          page,
    ...
    await selectDropdownOptionByLabel(
      page,
      selectors.previousYearNCBDropdown,
      insuranceDetails.previousNcb
    );
    await pendingXHR.waitForAllXhrFinished();
    // Select previous insurer
    await selectDropdownOptionByLabel(
      page,
      selectors.previousInsurerDropdown,
      insuranceDetails.previousOdInsurer
    );
    await pendingXHR.waitForAllXhrFinished();
    // Enter previous policy number
    await setElementValue(
      page,
      selectors.previousPolicyNumberInput,
      insuranceDetails.previousOdPolicyNumber
    );
    ...
    await setElementValue(
      page,
      selectors.previousOdPolicyStartDateInput,
      dayjs(insuranceDetails.previousOdEndDate)
        .add(1, 'day')
        .subtract(1, 'year')
        .format('DD/MM/YYYY')
    );
    await setElementValue(
      page,
      selectors.previousOdPolicyEndDateInput,
      dayjs(insuranceDetails.previousOdEndDate).format('DD/MM/YYYY')
    );

    // Enter previous tp policy start and end date
    await setElementValue(
      page,
      selectors.previousTpPolicyStartDateInput,
      dayjs(insuranceDetails.previousTpEndDate)
        .add(1, 'day')
        .subtract(5, 'year')
        .format('DD/MM/YYYY')
    );
    await setElementValue(
      page,
      selectors.previousTpPolicyEndDateInput,
      dayjs(insuranceDetails.previousTpEndDate).format('DD/MM/YYYY')
    );

    // Enter number of claims
    insuranceDetails.insurer.data.noOfClaimInPreviousPolicy
      ? insuranceDetails.insurer.data.noOfClaimInPreviousPolicy
      : '0';
    await setElementValue(
      page,
      selectors.noOfClaimsInput,
      insuranceDetails.insurer.data.noOfClaimInPreviousPolicy
    );
    ...
    insuranceDetails.policyType === 'od1'
      ? await selectDropdownOptionByLabel(
          page,
          selectors.pastPolicyTenureDropdown,
          '5'
        )
      : await selectDropdownOptionByLabel(
          page,
...
const checkIdvVariation = async (page, selectors, value) => {
  const higherIdvValue = await getElementValue(
    page,
    selectors.higherIdvValue,
    true
  );
  const lowerIdvValue = await getElementValue(
    page,
    selectors.lowerIdvValue,
    true
  );
...
const getDiscountValue = async (
  ...
  if (policyType === 'od1tp5') {
    ...
    if (vehicleAge <= 3) {
      discountValue =
        discounts.renewal.vehicleAge[3].state[state]['cc'][discountCc][
          bodyType
        ];
    } else if (vehicleAge > 3 && vehicleAge <= 9) {
      discountValue =
        discounts.renewal.vehicleAge[9].state[state]['cc'][discountCc][
          bodyType
        ];
    } else if (vehicleAge > 9 && vehicleAge <= 12) {
      discountValue =
        discounts.renewal.vehicleAge[12].state[state]['cc'][discountCc][
          bodyType
        ];
    } else {
      throw new Error(
        "Policy can't be created because VEHICLE AGE is over 12 years. Hence, DECLINED BY MAGMA INSURER"
      );
    }
...
const getSelectorIdAndSelectDropdownValue = async (
  page,
  value,
  pendingXHR,
  include = false
  ...
  const selectorId = await page.evaluate((value) => {
    ...
    for (let list in lists) {
      try {
        const listId = lists[list].getAttribute('id')
          ? lists[list].getAttribute('id')
          : null;
        const valueFound = document
...
const setVehicleDetails = async (
  ...
  const { bodyType, manufacturer, registrationDate } = vehicleDetails;

  const { policyType } = insuranceDetails;

  // Enter registration date
  await page.waitForSelector(selectors.vehicleRegistrationDate);
  await setElementValue(
    page,
    selectors.vehicleRegistrationDate,
    dayjs(vehicleDetails.registrationDate).format('DD/MM/YYYY')
  );
  await pendingXHR.waitForAllXhrFinished();
  // Enter rto location
  ...
  await getSelectorIdAndSelectDropdownValue(
    page,
    rtoLocation,
    pendingXHR,
    true
  ...
  await selectDropdownOptionByLabel(
    page,
    selectors.monthDropdown,
    month.toString()
  );
  await selectDropdownOptionByLabel(page, selectors.yearDropdown, year);
  // Enter manufacturer
  await page.type(selectors.manufacturerInput, vehicleDetails.manufacturer, {
    delay: 100
  });
  ...
  // check idv variation
  await checkIdvVariation(
    page,
    selectors,
    insuranceDetails.premiumDetails.idv.basic,
    taskId,
    pendingXHR
  );
  // Set vehicle idv
  ...
  await setElementValue(
    page,
    selectors.chassisNumberInput,
    vehicleDetails.chassisNumber
  );
  // Enter engine number
  await setElementValue(
    page,
    selectors.engineNumberInput,
  ...
  const discountPercent = await getDiscountValue(
    bodyType,
    state,
    manufacturer,
    policyType,
  ...
  if (vehicleDetails.hypothecated === 'Yes') {
    ...
    await pendingXHR.waitForAllXhrFinished();
    await page.type(selectors.financierNameInput, vehicleDetails.financerName, {
      delay: 50
    });
    await getSelectorIdAndSelectDropdownValue(
      page,
      vehicleDetails.financerName,
      pendingXHR,
      false
    );
    ...
    await page.waitForSelector(
      `${selectors.financerAddressDropdown} > option:nth-child(2)`
    );
    ...
    await selectDropdownOptionByLabel(
      page,
      selectors.agreementTypeDropdown,
      'Hypothecation'
    );
    await pendingXHR.waitForAllXhrFinished();
...
const setAddOnPlansAndCalculatePremium = async (
  page,
  insuranceDetails,
  pendingXHR
) => {
  // Select zero dep if nil dep exists
  insuranceDetails.addonOdCoverages
    ? await selectDropdownOptionByLabel(
        page,
        selectors.addOnCoverPlanDropdown,
        'BasicPlus'
      )
    : null;
  await pendingXHR.waitForAllXhrFinished();
  ...
  if (
    insuranceDetails.policyType === 'od1tp5' ||
    insuranceDetails.policyType === 'od1tp1'
  ) {
  ...
  if (
    ...
    const checkDisabled = async (page, selector) => {
      const isDisabled = await page.evaluate(
        (selector) => document.querySelector(selector).getAttribute('disabled'),
        selector
      );
...
const paymentAndDownloadPdf = async (page, pendingXHR) => {
  ...
  await pendingXHR.waitForAllXhrFinished();
  await page.waitForNavigation();
  await page.waitForSelector(selectors.paymentOptionDropdown, {
    visible: true
  });
  await selectDropdownOptionByLabel(
    page,
    selectors.paymentOptionDropdown,
    'CD'
  );
  await pendingXHR.waitForAllXhrFinished();
  await page.waitForSelector(selectors.proceedToPaymentButton2, {
    visible: true
  });
...
const setNewVehicleCalculationsAndUpdateClickUpTask = async (
  page,
  request,
  taskId,
  pendingXHR
  ...
  //? Set required variables
  const { insuranceDetails, customerDetails, vehicleDetails } = request;
  ...
  const { od, tp } = caclulationsTemplate;

  const ccValue = await getElementValue(page, selectors.ccInputValue);
  // Get discount value
  const discountValue = await getDiscountValue(
    bodyType,
    state,
    manufacturer,
    policyType,
  ...
  const pdfUrl = await calculationsPdfWindow.evaluate(() => document.URL);
  ...
  const basicNetPremium =
    policyType === 'od1tp5' || policyType === 'od1tp1'
      ? roundToDecimal(
          parseFloat(caclulationsJson['Total Own Damage Premium(A)'])
        )
      : roundToDecimal(
          parseFloat(caclulationsJson['Total Own Damage Premium'])
        );
  const nilDepAmount = caclulationsJson[
    'Add On Cover Basic Plus- ( DEPRECIATION RE-IMBURSEMENT )'
  ]
    ? parseFloat(
        caclulationsJson[
          'Add On Cover Basic Plus- ( DEPRECIATION RE-IMBURSEMENT )'
  ...
  const ncbPercentage =
    policyType === 'od1tp5' || insuranceDetails.previousNcb === '0'
      ? 0
      : parseInt(insuranceDetails.previousNcb) + 5;
  const ncbAmount =
    policyType === 'od1tp5'
      ? 0
      : roundToDecimal(
          (basicOd - basicOd * (discountValue / 100)) * (ncbPercentage / 100)
        );
  const discountAmount =
    discountPercentage === 0
      ? 0
      : roundToDecimal(basicOd - (basicNetPremium - nilDepAmount + ncbAmount));
  ...
  const basicTp =
    policyType === 'od1tp5' || policyType === 'od1tp1'
      ? caclulationsJson['Sub Total']
        ? roundToDecimal(parseFloat(caclulationsJson['Sub Total']))
        : 0
      : 0;
  const tppd =
    policyType === 'od1tp5' || policyType === 'od1tp1'
      ? caclulationsJson['Less:TPPD Discount']
        ? -parseFloat(caclulationsJson['Less:TPPD Discount'])
        : 0
      : 0;
  const tpNetPremium = caclulationsJson['Total Liability Premium(B)']
    ? roundToDecimal(parseFloat(caclulationsJson['Total Liability Premium(B)']))
    : 0;
  ...
  finalNetPremium =
    policyType === 'od1tp5' || policyType === 'od1tp1'
      ? roundToDecimal(
          parseFloat(
            caclulationsJson['Premium Computation Total Package Premium(A+B)']
          )
        )
      : roundToDecimal(
          parseFloat(
            caclulationsJson['Premium Computation Total Package Premium']
          )
        );
  const finalGstPremium = roundToDecimal(parseFloat(caclulationsJson['GST']));
  const finalGrossPremium = roundToDecimal(
    parseFloat(caclulationsJson['TOTAL'])
  );
  ...
  od.odAfterDiscount =
    discountPercentage === 0
      ? 0
      : policyType === 'od1tp5'
      ? roundToDecimal(basicNetPremium - nilDepAmount)
      : roundToDecimal(basicOd - discountAmount);
...
const downloadDocumentFileGoogleDrive = async (
  requestId,
  fileId,
  documentType
) => {
  ...
  try {
    ...
    const [, fileExtension] = fileData.headers['content-type'].split('/');
    const destination = fs.createWriteStream(
      `${kycDocumentFolderPath}/${requestId}${documentType}.${fileExtension}`
    );
    ...
    service.files.get(
      ...
      (err, { data }) => {
        ...
        data
          ...
          .on('error', () => {
            console.log(
              'Error in creating the file. Please upload the KYC document file manually.'
            );
            return process.exit();
          })
          .pipe(destination);
...
const downloadKycDocuments = async (request) => {
  //? Set Required Variables
  const { customerDetails, requestId } = request;

  const { aadhaarFileId, panFileId } = customerDetails;
  ...
  if (request.customerDetails.customerType === 'Individual') {
    aadhaarFileName = await downloadDocumentFileGoogleDrive(
      requestId,
      aadhaarFileId,
      'aadhaar'
    );
  ...
  // Download pan file
  const panFileName = await downloadDocumentFileGoogleDrive(
    requestId,
    panFileId,
    'pan'
  );
