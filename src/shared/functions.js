...
const getCredentials = async (insurer, branch) => {
  const getCredsUrl =
    'https://wandering-overcoat-duck.cyclic.app/aegiscovenant/generic-credentials';
...
const checkElementExists = async (page, selector) =>
  await page.evaluate(
    (selector) => (document.querySelector(selector) ? true : false),
    selector
  );

// Function to get the value of an element
const getElementValue = async (
  page,
  selector,
  numeric = false,
  type = 'input'
...
const selectDropdownOptionByLabel = async (
  page,
  selector,
  value,
  minOptions = 2
...
const getDriveCreds = async () => {
  const data = await fetch(
    'https://wandering-overcoat-duck.cyclic.app/aegiscovenant/drive-credentials'
  );

  const creds = await data.json();
  fs.writeFileSync('./driveCredentials.json', creds, () => {
...
const deleteGoogleDriveCredsFile = async () => {
  fs.unlinkSync(path.resolve('driveCredentials.json'), () => {
