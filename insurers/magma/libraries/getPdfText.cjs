...
const scrapper = async (text, policyType = 'od1tp5') => {
  ...
  if (policyType === 'od1tp5' || policyType == 'od1tp1') {
    ...
    const removeNewLine = splitText[1].replace(/,/g, '').replace(/(\r\n|\n|\r)/gm, '')
    const addNewLine = removeNewLine.replace(/[+-]?([0-9]*[.])?[0-9]+/g, '$&\n')
    const splitByNumber = addNewLine.split('\n');

    splitByNumber.forEach(element => {
        element = element.trim();
        const splitLabelAndValue = element.match(/\b([^\d\.]+)\b|\b(\d+\.\d+)\b/g);
        if (splitLabelAndValue.length > 1) {
            finalOutput[splitLabelAndValue[0].trim()] = splitLabelAndValue[splitLabelAndValue.length - 1];
        }
    });
  } else {
    ...
    splitByNumber.forEach(element => {
        ...
        if (splitLabelAndValue !== 'null' && splitLabelAndValue !== null && splitLabelAndValue.length > 1) {
            finalOutput[splitLabelAndValue[0].trim()] = splitLabelAndValue[splitLabelAndValue.length - 1];
        }
    });
  }
...
const extractPDF = async(url, policyType) => {
    const fileData = await axios.get(url, {responseType: 'arraybuffer'}, {headers: {
