...
const integromatSendBufferHook = 'https://hook.integromat.com/olwog4nmr62r36xkbk1n90fm28smuj6j';
...
const od1tp5PeriodOfOdInsurance = now.format('DD/MM/YYYY (HH:mm) [HRS]')
                                  + ' - '
                                  + now.add(1, 'year').subtract(1, 'day')
                                    .format('DD/MM/YYYY [Midnight]');
const od1tp5PeriodOfTpInsurance = now.format('DD/MM/YYYY (HH:mm) [HRS]')
                                  + ' - '
                                  + now.add(5, 'years').subtract(1, 'day')
                                    .format('DD/MM/YYYY [Midnight]');
...
const od1NotExpiredPeriodOfOdInsurance = notExpired.format('DD/MM/YYYY (HH:mm) [HRS]')
                                         + ' - '
                                         + notExpired.add(1, 'year').subtract(1, 'day')
                                           .format('DD/MM/YYYY [Midnight]');

// OD1 - Expired
const od1ExpiredRiskStartDate = expired.format('YYYY/MM/DD HH:mm:ss');
const od1ExpiredPeriodOfOdInsurance = expired.format('DD/MM/YYYY (HH:mm) [HRS]')
                                      + ' - '
                                      + expired.add(1, 'year').subtract(1, 'day')
                                        .format('DD/MM/YYYY [Midnight]');
